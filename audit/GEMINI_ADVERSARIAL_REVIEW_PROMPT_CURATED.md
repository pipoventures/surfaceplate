...
hidden.** The complete framework is roughly 170 files; attached here (`EVIDENCE_BUNDLE.md`) are 27,
chosen as the minimum needed to test the claims below directly rather than from a description of
them. Your coverage is genuinely partial — treat anything you cannot establish from these 27 files
...
- `surfaceplate/seeds/*` — **the sixteen documents `scaffold.py` actually writes** (the fifth, the adoption decision record, since `ACT-044`; the sixth, the findings register, since `ACT-052`; ten more, every gate and control whose artefact can be an honest empty register, since `ACT-053`). These are included
...
1. Inspect `EVIDENCE_BUNDLE.md` in full — all 27 files, each under its own `## FILE:` heading. Do
...
2. **The manifest anchor — attempt this ONLY if you can execute code.** The previous pass of this
   ...
   **Report this as a fact, not a verdict.** Two cautions the maintainer would rather you had than
   not: you cannot verify the rest of the archive from a 27-file subset, only this one value; and
...
4. Distinguish clearly, in every claim you make:
   ...
   - `EVIDENCE GAP`: not established by the 27 files given — including anything that would need one
