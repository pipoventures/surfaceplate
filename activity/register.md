...
| `ACT-052` | An explicit "create it" choice where a seed exists, every artefact offered with matches first, a seed for the findings register, and `adopt --edit` for a post-write edit (`DR-54`) | maintainer | maintainer | `in_progress` | `ACT-051` | hard | yes — what the interview offers, and one flag | low — each item has a regression test; the offer is judged on the images | `F86`, `F87`, `F88` closed test-first; the gate list and the remainder form showing the create-it row at 80×24, images read; goldens regenerated for cause; all fifteen CI steps green | The pull request; the images; the golden delta |
