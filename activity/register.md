...
| `ACT-066` | `F122` and `F123`: the Copilot channel becomes declinable at install with the declining left as a trace, and `authority.md` stops naming one vendor's file as where an authority hierarchy must live | maintainer | maintainer | `planned` | `ACT-063` | medium | yes — it changes what every adopting repository receives and edits an instruction shipped to every agent | medium — the mechanism follows `DR-29`'s `--no-hooks` precedent; the judgement is what the agent-agnostic wording should name instead | An adopter may decline the Copilot channel, the check reports on every run that it was declined, and no instruction names a vendor-specific file as the authority map | The declining seen to leave a trace and a silent decline shown to be impossible; the emitted payload listed with and without the flag; all fourteen suites |
