...
| `ACT-055` | The gate list opens with the level's floor expanded and the rest folded under one counted heading; the counter and the above-floor list name the floor (`F91`, `DR-56`) | maintainer | maintainer | `in_progress` | `ACT-054` | hard | yes — what the interview shows | low — a fold with a regression test; judged on the image | At 80×24 the floor and the fold heading on screen and no folded body; the key opens it; the join holds folded or open; the golden regenerated for cause; all fifteen CI steps green | The pull request; the image; the golden delta |
