...
| `ACT-053` | A seed and the "create it" row for every gate and control whose artefact can be an honest empty register, log or statement of the standard's rule: seven more gates and the four record-directory controls (`DR-55`) | maintainer | maintainer | `waiting_for_review` | `ACT-052` | hard | yes — what the interview offers | low — every seed is judged by reading it | Every seed writes, holds no placeholder and says it holds nothing; a `full` run choosing every row writes them all and the checker passes; the shared directory yields one note; all fifteen CI steps green | The pull request; the seeds themselves. Delivered on `claude/act-053-every-honest-seed` (2026-09-02): ten seed files for eleven gate and control artefacts (two controls share a directory), test-first; a `full` run choosing every row writes them all and the checker passes in full. Awaiting the maintainer's review |
