...
| `ACT-056` | Continuing past the folded gates: Ctrl+S with folded undecided gates asks one question, all not applicable as one act or open them (`F96`, `DR-57`) | maintainer | maintainer | `in_progress` | `ACT-055` | hard | yes — what the interview asks at one moment | low — one screen, one regression test | The question appears only when folded gates are undecided; yes records the bulk act with its count and continues; no opens the fold on the first undecided gate; quit changes nothing; all fifteen CI steps green | The pull request; the test |
