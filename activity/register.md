...
| `ACT-054` | Record the findings of the maintainer's third run (`F91` to `F94`); close `F93` and `F94` under `DR-51` (5) and fix `F92`'s message | maintainer | maintainer | `waiting_for_review` | `ACT-053` | hard | yes — what is proposed, and one checker message | low — proposal rules with regression tests | `F91` and `F92` recorded with the maintainer's words; `F93` and `F94` closed test-first; the `SP034` message prints each value as declared; the vendored checker reinstalled and the digest re-pinned; all fifteen CI steps green | The pull request; the register checks. Delivered on `claude/act-054-third-run` (2026-09-02), test-first; a seeded directory vanishing from the offer found and fixed on the way. Awaiting the maintainer's review |
