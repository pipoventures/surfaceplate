...
| `ACT-051` | The mark on the opening screen (`F89`, `DR-53`), and the timing-sensitive render test that turned `main` red (`F90`) | maintainer | maintainer | `waiting_for_review` | `ACT-050` | hard | yes — what the opening screen shows | low — the maintainer chose the mark on previews | The slab generated from its geometry with a test on the geometry; the opening screen carrying it within 24 rows with the draft note, asserted; the golden regenerated for cause; the render test waiting for the layout rather than assuming frames; all fifteen CI steps green | The pull request; the image read; the golden delta |
