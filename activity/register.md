...
<<<<<<< HEAD
| `ACT-058` | Reserve the package name on PyPI with the current version, by trusted publishing, keeping the git form in every instruction until 1.0 (`DR-61`) | maintainer | maintainer | `external_dependency` | `ACT-057` | external | no — a reservation; nothing an adopter runs changes | low — the decision is `DR-61`'s; the dispatch is the maintainer's | The publish workflow merged; the maintainer's PyPI account and pending trusted publisher in place; the workflow dispatched once and the project page live with the alpha classifier | The workflow run; the PyPI project page |
| `ACT-059` | Remediate the second cross-provider review (`audit/CROSS_PROVIDER_REVIEW_2026-09-02_PASS2.md`): implement what `H13` decides for `F101` to `F111`, test-first, and record what is declined and why | maintainer | maintainer | `in_progress` | `ACT-057` | hard | yes — the wizard's output, a public schema, and this repository's own profile are among the candidates | high — every item is a decision the maintainer takes first | Each of `F101` to `F111` closed by a change with a regression, or by a recorded decision not to change; the review's evidence gaps carried into the audit packet | The findings' closure paragraphs; the pull requests |
=======
