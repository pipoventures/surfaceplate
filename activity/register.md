...
| `ACT-049` | Close what an agent can close before 1.0: `F67`'s remainder (the level options below the fold, off-state and focus contrast, the above-floor labels, the classification rows, text-area height), `F73` (cancel reachable) and `F77`'s remainder (atomic profile write, a draft with stale ids); record `DR-52` and close `H2` | maintainer | maintainer | `in_progress` | `ACT-048` | hard | yes — it changes what screens show | low — each item has a regression test; the visual items are judged on the images | Each item test-first, seen to fail; every screen fits its purpose at 80×24 with the images read; the goldens regenerated for cause; `F67`, `F73`, `F77` closed; `H2` closed under `DR-52`; item 5 rewritten; all fifteen CI steps green | The pull request with commands and counts quoted; the images read; the golden delta per screen |
