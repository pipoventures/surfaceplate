...
| `ACT-048` | Remediate `adopt` for a first-time reader: version parity before the first question, an opening screen, a stated minimum on every field, described choices, proposals the checker accepts, honest refusals and closing report (`DR-51`) | maintainer | maintainer | `in_progress` | `ACT-047` | hard | yes — it changes what the interview shows and proposes | medium — bounded by `DR-51`; the authored minimum is judged by reading | `F78` to `F85` closed with a regression test each, seen to fail first; every presented field at every level carries help, what it decides and what a wrong answer costs; the parity table gains `SP046` and `SP032` rows a validator meets; the snapshot goldens regenerated with `DR-51` as the cause and the delta recorded; all fifteen CI steps green from `.venv/bin/python` | The pull request with commands and counts quoted; the failing-then-passing run of each test; the snapshot delta named per screen |
