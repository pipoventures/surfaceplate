...
| `ACT-050` | Record the findings of the maintainer's second run of `adopt` (`F87` to `F89`) | maintainer | maintainer | `waiting_for_review` | `ACT-049` | hard | no — records only | low | `F87` to `F89` in `org/FINDINGS.md` with the maintainer's words and a remedy hypothesis each | The pull request; register checks green |
