...
| `ACT-057` | Combinatorial validation of `adopt`: every reachable decision run against a real repository, a property oracle, a generated report and a signed record (`DR-58`) | maintainer | maintainer | `in_progress` | `ACT-056` | hard | no — validation; any defect found is its own finding | medium — the coverage definition is `DR-58`'s; the record's outcome is the maintainer's | The matrix suite enumerated from the catalogue with a coverage self-check; every case's oracle green or its expected findings exact; the report tracked and `--check` green; the assurance record validating against the schema; the sixteenth CI step green and confirmed by the loop; `CLAUDE.md` says fourteen | The pull request with the counts and the run time; the report; the record |
