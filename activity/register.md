...
| `ACT-043` | Remediate `adopt`, phase 8: the eight defects that make the wizard unfinishable or destructive | maintainer | maintainer | `done` | `ACT-042` | hard | yes — it changes what every adopter sees on the gates screen and whether a real profile can be overwritten | low — every remedy is fixed by the finding; no rule changes | The gate status radios render at 80×24; the gates screen is seeded on the defaults route; an empty choice is refused at the field; a real profile is never mistaken for the template; `None` is blank; a validation error survives the focus move; quitting at the resume prompt keeps the draft; the resume header renders; help text is styled (`org/REMEDIATION_PLAN.md` §4 phase 0) | Each regression test seen to fail before its fix and pass after; the eight images from the review re-taken and read; `H1` re-run by the maintainer. Delivered on `claude/act-043-phase-0` (2026-09-02): eight commits, one per item, each with its test seen failing then passing and its finding closed in the same commit; `F59`, `F60`, `F63`, `F64`, `F68`, `F74` closed, `F67` closed for its help-text part, `F73` left open with the reason recorded; items 0.3 and 0.2 landed in that order because 0.2's hint assertion depends on 0.3; the fourteen CI steps run from `.venv/bin/python` are quoted in the pull request. Awaiting the fresh-session review and the maintainer's `H1` run. **Done 2026-09-02.** Reviewed and merged as PR #44 (`6d5c110`); the maintainer's `H1` run of the same day completed the wizard against Plutos (plutos#6), the acceptance evidence this row named |
| `ACT-044` | Remediate `adopt`, phase 9: proposal-first adoption with a provenance record, one gate list, validator parity, discovery that excludes the framework | maintainer | maintainer | `done` | `ACT-043` | hard | yes — it changes what every adopter is asked, what the profile says about its own origin, and what discovery may propose | high — bounded by `DR-47` and `DR-48`; the level stays a human decision; no scope decision is ever supplied by the tool | Eight to twelve fields before the review on the two fixtures the prototype used; the provenance record written and read by `test_provenance.py`; the header true; a gate list with no pre-marked statuses and a recorded bulk command; a profile that passes the wizard passes its first check; no framework-owned path proposed (plan §4 phase 1) | The persona budget measured, not estimated; the prototype's profile equality reproduced on the real code; the `SP` parity test green; a repository containing only the install payload yielding no proposals. Delivered on `claude/act-044-phase-1`, stacked on phase 0 (2026-09-02): items 1.6, 1.4, 1.5 as their own commits, then the flow (1.1, 1.2, 1.3, 1.7, 1.8) as one; budget measured at 11 fields before the review on the discoverable standard fixture and 12 on the bare essential one (`tests/test_adopt.py`); profile equality reproduced; parity table green (14 codes met, 42 exempt); the payload-only repository yields nothing; `F61`, `F62`, `F65`, `F66`, `F69`, `F75`, `F76` closed, `F77` two items closed; the fourteen CI steps quoted in the pull request. Awaiting the fresh-session review. **Done 2026-09-02.** Reviewed and merged as PR #45 (`a0053d1`); the maintainer's `H1` run exercised the new flow end to end and its provenance record (plutos#6) |
| `ACT-045` | Remediate `adopt` and `check`, phase 10: non-interactive adoption, `doctor`, output formats and exit codes, the bounded front-door pass, generated numbers and link checks | maintainer | maintainer | `done` | `ACT-044` | hard | yes — a public exit-code contract, a new command, and the first sentence a stranger reads | medium — bounded by `DR-49` and the release plan's item 8 | `adopt --propose` and `--answers` round-trip the prototype's answers to the same profile; `doctor` reports the five facts that stopped the stranger install; `check --format json` and `sarif` validate; exit codes as `DR-49` decides; README and `INSTALL.md` as bounded; every number and link in the documents checked by `check_code_registers.py` in both this repository and an installed checkout (plan §4 phase 2) | A clean-container run of every documented command, on a machine with a global `core.hooksPath`; a SARIF file accepted by a validator; the answers file replayed by CI. Delivered on `claude/act-045-phase-2`, stacked on phase 1 (2026-09-02): `scripts/front_door.sh` run in `python:3.12-slim` with a global hooks path, `FRONT_DOOR=PASS`, and wired as `.github/workflows/front-door.yml`; the SARIF output validated against the OASIS 2.1.0 schema with no errors in the graced and failing states; the answers record replayed by `tests/test_adopt.py` in the adopt suite; `F70` closed, `F71` five of six (the Actions claim waits for `H10`), `F77` five more items; the fourteen CI steps quoted in the pull request. Awaiting the fresh-session review. **Done 2026-09-02.** Reviewed and merged as PR #46 into the phase 1 branch, landing on `main` with PR #45; the definition of done is met in full; `F71`'s last item, the Actions claim, stays with `H10` and is not part of it |
| `ACT-046` | Remediate the evidence machinery, phase 11: the 80×24 snapshot suite and budget test, register parity, the schema change, docstring style | maintainer | maintainer | `done` | `ACT-045` | hard | yes — a schema change and a new golden suite | medium — bounded by `DR-50`; snapshots are goldens and never regenerated to absorb an unexplained diff | Every screen snapshot-tested at 80×24 including a focused gate status; the budget test asserting the measured numbers; `check_code_registers.py` asserting body-versus-index status; the schema change with both examples updated; the suite count in `CLAUDE.md` and the workflow's "every step ran" loop updated in the same change (plan §4 phase 3) | The snapshot suite seen to fail on a one-character stylesheet change; the parity check seen to fail on one deliberately contradicted status in a scratch copy. Delivered on `claude/act-046-phase-3`, stacked on phase 2 (2026-09-02): nine 80×24 snapshots including a gates list with a focused status, seen to fail on `#7a827e` → `#7a827f` (2 of 9); the parity check seen to fail on the ten live contradictions before they were reconciled; the budget asserted at the measured 11 and 12; the schema change with both examples and the template; `DR-50` (3) the draft under `.standards/`; the `test` extra reviewed (pytest 8.4.2 MIT, pytest-textual-snapshot 1.1.0 MIT, syrupy 4.8.0 Apache-2.0, `pip check` clean); `CLAUDE.md` says thirteen and the workflow's loop gains `snapshots`; `F72` closed, `F77` three more items. Awaiting the fresh-session review. **Done 2026-09-02.** Reviewed and merged as PR #47 (`35e6222`); the snapshot suite ran green on its first CI run, closing the cross-machine determinism gap the PR recorded |
| `ACT-047` | Record the findings of the maintainer's first `H1` run of `adopt` against Plutos (`F78` to `F82`) and open the decision they need | maintainer | maintainer | `done` | `ACT-046` | hard | no — it records only | low — what was observed and said, in the maintainer's own words; the remedy design is `H11`'s | `F78` to `F82` in `org/FINDINGS.md` with index and body in parity; `H1` rewritten with the unblock and the residue it now meets; `H11` opened for the decision and the remediation activity | PR #48, merged to `main` as `f9e063a` (2026-09-02); `tests/check_code_registers.py` parity green; the maintainer's agreement to the five findings in the recording session (https://claude.ai/code/session_01Bz6QZWcsg9tRFuH9gS331Z) |
| `ACT-048` | Remediate `adopt` for a first-time reader: version parity before the first question, an opening screen, a stated minimum on every field, described choices, proposals the checker accepts, honest refusals and closing report (`DR-51`) | maintainer | maintainer | `done` | `ACT-047` | hard | yes — it changes what the interview shows and proposes | medium — bounded by `DR-51`; the authored minimum is judged by reading | `F78` to `F85` closed with a regression test each, seen to fail first; every presented field at every level carries help, what it decides and what a wrong answer costs; the parity table gains `SP046` and `SP032` rows a validator meets; the snapshot goldens regenerated with `DR-51` as the cause and the delta recorded; all fifteen CI steps green from `.venv/bin/python` | The pull request with commands and counts quoted; the failing-then-passing run of each test; the snapshot delta named per screen. Delivered on `claude/act-048-first-time-reader` (2026-09-02): six commits, one per item of `DR-51`, each with its tests seen to fail first; goldens regenerated for cause (decisions form, gates list, review with an error) and one created (the opening screen); Plutos's two checker findings fixed by hand and committed as plutos#6, after which `surfaceplate check` passes there in full. Awaiting the maintainer's review of the authored text. **Done 2026-09-02.** Reviewed and merged as PR #49 (`eba799c`) after fifteen green CI steps; the authored text was the maintainer's to read and is now on `main` |
| `ACT-049` | Close what an agent can close before 1.0: `F67`'s remainder (the level options below the fold, off-state and focus contrast, the above-floor labels, the classification rows, text-area height), `F73` (cancel reachable) and `F77`'s remainder (atomic profile write, a draft with stale ids); record `DR-52` and close `H2` | maintainer | maintainer | `done` | `ACT-048` | hard | yes — it changes what screens show | low — each item has a regression test; the visual items are judged on the images | Each item test-first, seen to fail; every screen fits its purpose at 80×24 with the images read; the goldens regenerated for cause; `F67`, `F73`, `F77` closed; `H2` closed under `DR-52`; item 5 rewritten; all fifteen CI steps green | The pull request with commands and counts quoted; the images read; the golden delta per screen. Delivered on `claude/act-049-close-what-closes` (2026-09-02): `DR-52` and `H2` first, then `F77` and `F73` test-first, then `F67`'s five items test-first with the images read; goldens regenerated for cause. Awaiting the maintainer's review. **Done 2026-09-02.** Reviewed and merged as PR #51 (`a6ebca6`); the maintainer's second run of the wizard on a scratch copy of Plutos completed and passed the checker in full |
| `ACT-050` | Record the findings of the maintainer's second run of `adopt` (`F87` to `F89`) | maintainer | maintainer | `done` | `ACT-049` | hard | no — records only | low | `F87` to `F89` in `org/FINDINGS.md` with the maintainer's words and a remedy hypothesis each | The pull request; register checks green. **Done 2026-09-02.** Merged as PR #52 (`9a6f71d`) |
| `ACT-051` | The mark on the opening screen (`F89`, `DR-53`), and the timing-sensitive render test that turned `main` red (`F90`) | maintainer | maintainer | `done` | `ACT-050` | hard | yes — what the opening screen shows | low — the maintainer chose the mark on previews | The slab generated from its geometry with a test on the geometry; the opening screen carrying it within 24 rows with the draft note, asserted; the golden regenerated for cause; the render test waiting for the layout rather than assuming frames; all fifteen CI steps green | The pull request; the image read; the golden delta. **Done 2026-09-02.** Reviewed and merged as PR #53 (`676e5bc`); main's self-check green again on it |
| `ACT-052` | An explicit "create it" choice where a seed exists, every artefact offered with matches first, a seed for the findings register, and `adopt --edit` for a post-write edit (`DR-54`) | maintainer | maintainer | `done` | `ACT-051` | hard | yes — what the interview offers, and one flag | low — each item has a regression test; the offer is judged on the images | `F86`, `F87`, `F88` closed test-first; the gate list and the remainder form showing the create-it row at 80×24, images read; goldens regenerated for cause; all fifteen CI steps green | The pull request; the images; the golden delta. Delivered on `claude/act-052-seeds-refs-edit` (2026-09-02), each item test-first; the wider offer surfaced a workflow file proposed as a findings register, fixed in the same change. Awaiting the maintainer's review. **Done 2026-09-02.** Reviewed and merged as PR #54 (`ec8fb3c`) |
| `ACT-053` | A seed and the "create it" row for every gate and control whose artefact can be an honest empty register, log or statement of the standard's rule: seven more gates and the four record-directory controls (`DR-55`) | maintainer | maintainer | `done` | `ACT-052` | hard | yes — what the interview offers | low — every seed is judged by reading it | Every seed writes, holds no placeholder and says it holds nothing; a `full` run choosing every row writes them all and the checker passes; the shared directory yields one note; all fifteen CI steps green | The pull request; the seeds themselves. Delivered on `claude/act-053-every-honest-seed` (2026-09-02): ten seed files for eleven gate and control artefacts (two controls share a directory), test-first; a `full` run choosing every row writes them all and the checker passes in full. Awaiting the maintainer's review. **Done 2026-09-02.** Reviewed and merged as PR #55 (`d4acd4e`) |
| `ACT-054` | Record the findings of the maintainer's third run (`F91` to `F94`); close `F93` and `F94` under `DR-51` (5) and fix `F92`'s message | maintainer | maintainer | `done` | `ACT-053` | hard | yes — what is proposed, and one checker message | low — proposal rules with regression tests | `F91` and `F92` recorded with the maintainer's words; `F93` and `F94` closed test-first; the `SP034` message prints each value as declared; the vendored checker reinstalled and the digest re-pinned; all fifteen CI steps green | The pull request; the register checks. Delivered on `claude/act-054-third-run` (2026-09-02), test-first; a seeded directory vanishing from the offer found and fixed on the way. Awaiting the maintainer's review. **Done 2026-09-02.** Reviewed and merged as PR #56 (`9a1c0ff`) |
| `ACT-055` | The gate list opens with the level's floor expanded and the rest folded under one counted heading; the counter and the above-floor list name the floor (`F91`, `DR-56`) | maintainer | maintainer | `done` | `ACT-054` | hard | yes — what the interview shows | low — a fold with a regression test; judged on the image | At 80×24 the floor and the fold heading on screen and no folded body; the key opens it; the join holds folded or open; the golden regenerated for cause; all fifteen CI steps green | The pull request; the image; the golden delta. Delivered on `claude/act-055-the-floor-first` (2026-09-02), test-first; the golden regenerated for cause. Awaiting the maintainer's review. **Done 2026-09-02.** Reviewed and merged as PR #57 (`4ca5f2a`) |
| `ACT-056` | Continuing past the folded gates: Ctrl+S with folded undecided gates asks one question, all not applicable as one act or open them (`F96`, `DR-57`) | maintainer | maintainer | `done` | `ACT-055` | hard | yes — what the interview asks at one moment | low — one screen, one regression test | The question appears only when folded gates are undecided; yes records the bulk act with its count and continues; no opens the fold on the first undecided gate; quit changes nothing; all fifteen CI steps green | The pull request; the test. Delivered on `claude/act-056-continue-past-the-fold` (2026-09-02), test-first. Awaiting the maintainer's review. **Done 2026-09-02.** Reviewed and merged as PR #58 (`fb71e3b`) |
| `ACT-057` | Combinatorial validation of `adopt`: every reachable decision run against a real repository, a property oracle, a generated report and a signed record (`DR-58`) | maintainer | maintainer | `done` | `ACT-056` | hard | no — validation; any defect found is its own finding | medium — the coverage definition is `DR-58`'s; the record's outcome is the maintainer's | The matrix suite enumerated from the catalogue with a coverage self-check; every case's oracle green or its expected findings exact; the report tracked and `--check` green; the assurance record validating against the schema; the sixteenth CI step green and confirmed by the loop; `CLAUDE.md` says fourteen | The pull request with the counts and the run time; `audit/validation/ADOPT_MATRIX.md` (208 cases, 45,257 checks, byte-stable across two runs); `governance/assurance/AE-0001-adopt-matrix.yaml`, whose outcome the maintainer sets by merging (H12); `F97` to `F100` found and closed; all sixteen steps PASS locally from `.venv` and on the runner (run 33682871213, and again on the final commit); merged by the maintainer as PR #60 at `bc2fd94` on 2026-09-02, which set the record's outcome (H12 taken) |
| `ACT-058` | Reserve the package name on PyPI with the current version, by trusted publishing, keeping the git form in every instruction until 1.0 (`DR-61`) | maintainer | maintainer | `done` | `ACT-057` | external | no — a reservation; nothing an adopter runs changes | low — the decision is `DR-61`'s; the dispatch is the maintainer's | The publish workflow merged; the maintainer's PyPI account and pending trusted publisher in place; the workflow dispatched once and the project page live with the alpha classifier | Run 33697386488 (build and publish both green, 2026-09-03); https://pypi.org/project/surfaceplate/0.16.0/ with the alpha classifier; installed from the index into a clean environment and run |
| `ACT-059` | Remediate the second cross-provider review (`audit/CROSS_PROVIDER_REVIEW_2026-09-02_PASS2.md`): implement what `H13` decides for `F101` to `F111`, test-first, and record what is declined and why | maintainer | maintainer | `done` | `ACT-057` | hard | yes — the wizard's output, a public schema, and this repository's own profile are among the candidates | high — every item is a decision the maintainer takes first | Each of `F101` to `F111` closed by a change with a regression, or by a recorded decision not to change; the review's evidence gaps carried into the audit packet | The findings' closure paragraphs; PRs #67 to #74, each green on the runner; `F101` to `F111` all closed on 2026-09-02/03 - eight by a change with a regression seen to fail first, `F102` by an advisory the maintainer chose over the reviewer's remedy, `F111` by the maintainer keeping the practice; the digest recomputation (`H4`) stays an evidence gap |
| `ACT-060` | The independent review packet for `H4` and `H6` (`DR-64`): a tracked source, a generator that builds one self-contained page and refuses an archive from the wrong tree, the audit-packet check extended; `F114` closed; `F115` recorded | maintainer | maintainer | `done` | `ACT-059` | hard | no — nothing an adopter runs changes | medium — `DR-64` fixes what closes `F6`; the reviewer's attestation is the outside party's | The source and generator merged; `tests/check_audit_packet.py` green having failed first on the counts and the guard; the page generated for 0.16.0 with the published values and its digest quoted; `H4`/`H6` prepared columns carrying the invocation and the return path | The check's output; the generated page's digest; the PR |
| `ACT-061` | The package's public links (`[project.urls]` for PyPI) and the README's front door made true: one adopting repository, and a publisher installed on itself (`F116`); a listing text for pipoventures.com delivered to the maintainer | maintainer | maintainer | `done` | `ACT-060` | hard | no — packaging metadata and prose | low | `[project.urls]` present and parseable; the three README sentences corrected; `F116` closed; the front-door checks green | The PR; `tests/check_code_registers.py`'s output |
| `ACT-062` | Three stale self-descriptions corrected (`README.md`'s PyPI status, `SECURITY.md`'s privacy claim: `F117`, `F118`) and a feedback surface built where none existed (`F119`): `about.ISSUES` threaded to every stdout surface a user reads on failure, `surfaceplate doctor --report` assembling a paste-ready, offline, redacted problem report, `SUPPORT.md` and two GitHub issue forms; plus the independent-review packet's distribution route (`audit/REVIEW_INVITATION.md`, a "Reviewing this" section in the README) so `H4`/`H6`'s prepared packet reaches an actual reviewer for the first time | maintainer | maintainer | `waiting_for_review` | `ACT-061` | hard | yes — `about.ISSUES` and `doctor --report` are new user-facing surfaces, and the three corrected claims are what every reader of this project's public documents sees first | medium — what to redact from a problem report, and how to phrase three corrected claims without over- or under-stating what changed, are judgement calls; threading a constant through existing print statements is not | `F117`–`F119` closed; `doctor --report` refuses `--online`, completes with the socket layer replaced by one that raises, and never emits the sentinel repo path, home directory, or username seeded in its test; `--report-file` writes exactly what stdout printed; `tests/test_adopt.py`'s pyproject binding extended; the review packet distributed via a GitHub Release rather than sitting outside the repository | [PR #82](https://github.com/pipoventures/surfaceplate/pull/82); the new checks in `tests/test_install_and_check.py` and `tests/test_adopt.py`; the [release](https://github.com/pipoventures/surfaceplate/releases/tag/pypi/0.16.1), published 2026-09-03 with the maintainer's approval of its exact notes (`H16`, half taken — posting the invitation is the remaining half) |
| `ACT-063` | Raise the six operating-model review findings as `F120` to `F125`, each re-verified against this repository before recording | maintainer | maintainer | `done` | — | review | no — it records defects; it changes nothing the framework does | medium — the judgement is whether each proposed finding survives re-verification, and how to record the two whose evidence did not | Six findings exist in `org/FINDINGS.md` with index and body agreeing, and `tests/check_code_registers.py` passes; every claim re-checked against the live tree, with each disagreement stated in the finding rather than silently corrected | The re-verification commands and their output for all six; the two corrections recorded in `F122` and `F125` naming what the review examined and what it should have examined |
| `ACT-064` | `SP038` dissolved by implementing `DR-1` in full: the installer refuses a foreign hooks path by default, an adopter may declare a chain, and the checker verifies **by effect** that the declared chain reaches this standard's own gate — a probe run, not a reading of the hook's text (`DR-66`) | maintainer | maintainer | `done` | — | hard | yes — it changes the installer, a published schema, and what a conformance result establishes about local enforcement, in every adopting repository | high — it is a security-boundary control, and a wrongly composed chain is worse than an honest refusal because it reads as present while running nothing | `check_conformance.py` returns `PASS` with no findings on this repository with the declared chain in place; a chain reaching a different `pre-commit` fails; an undeclared foreign path still refuses at install; `schema_version` is unchanged | A decision record implementing `DR-1`; all four hook scenarios seen to pass and fail in a scratch repository; the resolved hooks path read directly rather than inferred; all fourteen suites and the release ritual in its required order |
| `ACT-065` | The four rules the standard lacks, authored into the current instruction layer: the author-side supersession rule, the loop-breaker, the codification threshold, and a concurrency document that did not exist. The instruction set becomes seven, and every place that stated six is corrected in the same change | maintainer | maintainer | `done` | `ACT-064` | hard | yes — it adds four obligations to what every adopting repository receives | high — each rule arrives from a private doctrine bundle written for one machine and must be rewritten to be intelligible to a stranger who has read only this standard | Four rules present in the authored tree and emitted to both agent channels after a clean reinstall; no shipped sentence cites a repository, incident or machine an adopter cannot look up; the file counts stated in `README.md` and `AGENTS.md` corrected in the same change | The probes that established each gap, re-run and shown to return content where they returned none; the emitted files listed after reinstall; `MANIFEST.sha256` and `INSTALL.json` accounted for; all fourteen suites |
| `ACT-066` | `F122` and `F123`: **every** agent channel becomes declinable at install — not the Copilot one alone, which would be the same neutrality breach one layer along — with the declining recorded and reported, and `authority.md` stops naming one vendor's file as where an authority hierarchy must live (`DR-67`) | maintainer | maintainer | `done` | `ACT-063` | medium | yes — it changes what every adopting repository receives and edits an instruction shipped to every agent | medium — the mechanism follows `DR-29`'s `--no-hooks` precedent; the judgement is what the agent-agnostic wording should name instead | An adopter may decline the Copilot channel, the check reports on every run that it was declined, and no instruction names a vendor-specific file as the authority map | The declining seen to leave a trace and a silent decline shown to be impossible; the emitted payload listed with and without the flag; all fourteen suites |
| `ACT-067` | `F124`: a currency check, reporting installed-versus-published as an advisory where the network is, never as a failure (`DR-68`). Mechanism built and verified in all three directions; the **trigger** is left to `H20`, because a step in the installed workflow would call out from every adopter's CI and could not be removed by them | maintainer | maintainer | `done` | `ACT-063` | hard | yes — it adds a signal every adopting repository will see, about a property nothing currently reports | high — the control must report staleness without implying that a pinned version is a defect, and it runs where a network exists rather than inside the checker, which has none | An install one version behind produces a signal and a current install produces none — both directions demonstrated, because a check that fires on everything has detected nothing | Both directions run and their output quoted; the offline guarantee of `check_conformance.py` shown to be unchanged; all fourteen suites |
| `ACT-068` | The topic restructure across five surfaces — documents, wizard, application-profile schema, conformance levels and finding codes — carrying `WI-1` (`DR-2`'s scope amendment), `WI-2` (adopter-declared canon), `F120`'s level grading and `F125`'s precedence rule | maintainer | maintainer | `planned` | `ACT-064`, `ACT-065` | hard | yes — it changes the published profile contract, every document an adopter receives, and what the wizard asks | high — it is a methodology change, the taxonomy's boundaries are a human judgement, and the profile schema is a contract that breaks for adopters | Twelve topic documents each carrying a normative and an imperative part; the emitter ships only the imperative part to agent destinations; per-topic decisions in the schema; the wizard's order is the topic order; no finding code renumbered; the three constraints on baseline honesty, README calibration and claim strength verifiably unweakened | A decision record taken before any file moves, settling the taxonomy's contested boundaries and the collision between twelve topics and twelve controls; a cross-provider adversarial review of the documents and the schema; `ADOPT_MATRIX.md` and the golden SVGs regenerated only with the cause of each diff stated; all fourteen suites |
