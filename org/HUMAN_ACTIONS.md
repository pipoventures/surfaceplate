...
| H7 | **Decide forge neutrality** (item 3's remainder) | A scope decision about what the framework ships. `RELEASE_PLAN` calls it *"a larger question"*: `.github/skills/` and the conformance workflow are 8 of 57 installed files and assume GitHub | Item 3 moving from *largely done* to done | The measurement is in `RELEASE_PLAN` item 3. `DR-30` records how the agent-instruction half was solved, which is the pattern a decision here would follow |
...
| H9 | **Merge or reject the README front-door PR** | Prose on a public surface. `RELEASE_PLAN` sanctions the pass; the taste is yours, and there is no cost to it waiting | Nothing | Left as an open PR by design during `ACT-039`, never merged by an agent |
