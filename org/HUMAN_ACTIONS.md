...
| H8 | **Reserve the name on PyPI** (`DR-61`, option 2 chosen 2026-09-02); the git form stays the install route until 1.0 | A release decision with credentials attached. An agent may not take one, and `org/decisions/README.md` reserves release authority | `ACT-058`; the name staying free is not guaranteed | **Prepared: `.github/workflows/publish.yml`, trusted publishing, manual dispatch only.** Three steps, all yours: (1) create a PyPI account; (2) on PyPI, Your projects, Publishing, add a *pending* trusted publisher: project `surfaceplate`, owner `pipoventures`, repository `surfaceplate`, workflow `publish.yml`, environment `pypi`; (3) in this repository's Settings, Environments, create `pypi` (optionally with yourself as required reviewer), then run *Publish to PyPI* from the Actions tab typing `0.16.0`. No token is created or stored at any step |
...
| H13 | **Decide the remedies for the second cross-provider review** (`audit/CROSS_PROVIDER_REVIEW_2026-09-02_PASS2.md`; `F101` to `F111`): five design decisions (`F101` rollback, `F102` seed advisory, `F103` acceptance line, `F104`/`F105` schema tightening, `F108` scanner note), three edits to this repository's own profile to approve (`F106` wording, `F109`, `F110`), one fix to authorise (`F107`), one judgement (`F111`) | Each changes what the wizard writes, a public schema, or this repository's own declarations - decisions `DR-47`, `DR-49` and the operating model reserve to the maintainer | `ACT-059`, which implements what is decided; release-plan item 10 reads this review | Every finding carries an assessment and a proposed remedy or alternative in `org/FINDINGS.md`; the implementing session puts them one at a time |
