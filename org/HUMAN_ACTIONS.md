...
|---|---|---|---|---|
| H1 | **Run `adopt` against Plutos** (`RELEASE_PLAN` item 5). **Do not run until `ACT-043` has merged:** on the current build the gates screen cannot be completed (`F59`, `F60`) and a completed profile mentioning "replace-me" would be overwritten (`F63`) | It is a full-screen interactive wizard and needs a real terminal; every answer is a judgement about *your* repository — level, materiality, which artefacts are real | Item 5, and with it items 8, the second review, and 10. Item 5 is the last substantive item before the endgame | **Ready to start cold, 2026-09-02.** Plutos upgraded to current `main` (63 standard-owned files; the two missing against this repository's 64 are the hooks, which `--no-hooks` correctly skips) so it now has `.claude/skills/` from `ACT-041`. Its `governance/application-profile.yaml` is still the **untouched template** — every field `replace-me` — so this is a first run, not a repair. Command, verified to resolve from inside Plutos: `/home/mps2210/github/surfaceplate/.venv/bin/surfaceplate adopt`. **Started 2026-09-02 and stopped at the review by `F78`:** Plutos's install predates phase 3, so the assembled profile fails the *installed* schema on `risk`. Upgrade Plutos first — `/home/mps2210/github/surfaceplate/.venv/bin/surfaceplate install --source /home/mps2210/github/surfaceplate/surfaceplate --target /home/mps2210/github/plutos --no-hooks`, with `--dry-run` once before — then run `adopt` again and answer `y` to resume: the draft at `.standards/adopt-draft.json` holds all four stages' answers and is flagged stale because the digest changed, which is correct. **Residue you will meet:** `.surfaceplate-adopt-draft.json` from the run of 1 September is a format-2 draft, no longer offered and safe to delete; `governance/application-profile.yaml.first-attempt`, the unusable `asdf` profile, is read by nothing. Plutos's tree carries uncommitted installer output from both upgrades; it is regenerable and left for one commit after your run |
