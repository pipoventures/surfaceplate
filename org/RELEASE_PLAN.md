...
| 3 | Agent neutrality: `AGENTS.md` and per-agent emitters | `DR-12` commits to this; `DR-11` already decided the integrity mechanics (the `generated_files` structure) it depends on. Without it, "agent-neutral" is a claim this repository does not yet back with a working artefact. |
...
| 5 | Two real adopters: Plyego, Plutos | Neither appears anywhere in this repository today (confirmed by exhaustive grep). `DR-7` already established that every previously-recorded adopter was fabricated. 1.0 is genuinely the first time this framework is exercised against a repository that is not its own. |
...
3. **Neither named adopter (Plyego, Plutos) has agreed to anything.** Their presence in this plan is
   scope, not a commitment made on their behalf.
