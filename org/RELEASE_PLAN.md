...
| 5 | Two real adopters: Plyego, Plutos — **half begun** | `DR-7` established that every previously-recorded adopter was fabricated, and until 2026-08-31 neither name appeared anywhere in this repository. **Plyego has been exercised**: the framework was installed into a throwaway clone of it and returned `PASS` at `essential`, exposing two defects in surfaceplate that were unreachable from here (`F25`, `F26`) and running the four interface gates for the first time. `DR-28` records the result. **Plyego is not adopted** — nothing was written to it, and it is paused deliberately: Plyego is mid-migration to Google Cloud, and this is not the moment to install anything into it, probe or otherwise. **Plutos has now also been exercised**, in Plyego's place for this round, and passed cleanly at `essential` on the first attempt — no defect needed fixing to reach it, unlike Plyego. One defect was found and fixed regardless: `install_standard.py`'s `--no-hooks` path told every adopter to activate a hook it never installed. `DR-34` records the result and the per-gate cost. **Plutos is not adopted either** — nothing was written to it, same principle as Plyego: gate and control decisions are the maintainer's, not an agent's to infer from a probe. So this row is now 'two have been exercised, none has adopted'. |
...
1. **Items 0, 1, 2, 4, 6, 7 and 9 are done. Item 3 is largely done. Item 5 is half begun. Items 8
   and 10 do not exist in any form.** Done here means built, reviewed and merged — not
...
3. **Neither named adopter (Plyego, Plutos) has agreed to anything.** Their presence in this
   plan is scope, not a commitment made on their behalf. Both have been *exercised* — Plyego on
   2026-08-31, Plutos on 2026-09-01 — installed into a throwaway clone, never into the repository,
   which is a test the maintainer ran on his own projects, not an adoption anyone agreed to. `DR-28`,
   `DR-34`.
