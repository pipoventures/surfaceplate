...
integrity-checked in CI. Do not edit anything under `.standards/`, `.claude/rules/`, `.claude/skills/`, `.github/instructions/` and `.github/skills/` — raise changes against the
...
- **The standard's agent instructions are mandatory. Read them before acting.** Where anything in
  ...
  in the location that agent actually loads: `.claude/rules/surfaceplate-*.md` for Claude Code and `.github/instructions/*.instructions.md` for Copilot. If your agent reads none of those,
...
- **The skills define the workflow for each kind of task. Use the matching skill.** Its required
  ...
  agent, in the location that agent actually loads: `.claude/skills/*/SKILL.md` for Claude Code and `.github/skills/*/SKILL.md` for Copilot. Same body, same gates,
